$(function () {
  ...
  updateUserOnline = (listUsers) => {
    ...
    listUsers.map((value) => {
      ...
      content += "</div>";
      content +=
        '<h6 class="profile-username text-center">' + value.userName + "</h6>";
      content += "</div>";
      content += "</li>";
  ...
  socket.on("updateMessage", (data) => {
    ...
    if (data.sender === "You") {
      ...
      content +=
        '<img class="image-user imageUserBackground" src="' +
        data.imageLink +
        '" alt="user image">';
      ...
      content += '<div class="user-block">';
      content +=
        '<img class="image-user imageUserBackgroundOrder" src="' +
        data.imageLink +
        '" alt="user image">';
      content += '<span class="username">';
      content += '<a href="#">' + data.sender + "</a>";
    ...
    updateUserOnline(data.listUsers);
    $("#content-message-chat").animate(
      {
        scrollTop:
          $("#content-message-chat li.mottinnhan:last-child").last().offset()
            .top +
          ($("#content-message-chat li.mottinnhan:last-child").last().index() +
            S) *
            scrollTo,
      },
      2000
